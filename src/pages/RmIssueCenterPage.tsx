...
export default function RmIssueCenterPage() {
  ...
  const statuses = Array.from(new Set(allRows.map((r) => r.normalizedStatus))).sort();
  ...
  const openCount = rows.filter((r) => !["Complete", "Live"].includes(r.normalizedStatus)).length;
  ...
  const exportExcel = () => downloadCsv("cfs-rm-issues.csv", rows.map((r) => ({
    ...
    Description: r.description, Status: r.normalizedStatus, Urgency: r.urgency ?? "normal",
