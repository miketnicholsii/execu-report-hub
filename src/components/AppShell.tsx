...
const NAV = [
  ...
  { to: "/rm-issues", label: "RM Issue Center", icon: AlertTriangle, group: "Portfolio" },
  ...
  { to: "/setup", label: "Setup", icon: Target, group: "System" },
      ...
      <aside className={`${collapsed ? "w-14" : "w-56"} bg-sidebar border-r border-sidebar-border flex-shrink-0 transition-all duration-200 print:hidden sticky top-0 h-screen overflow-y-auto`}>
        <div className="p-3 flex items-center justify-between border-b border-sidebar-border">
          {!collapsed && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground font-medium mt-0.5">Intelligence</span>
            </div>
          )}
          ...
          <div className="px-2 pt-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-border bg-background pl-8 pr-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 transition-colors"
              />
            </div>
          </div>
        )}
          ...
          {GROUPS.map((group) => {
            ...
            return (
              <div key={group} className="mb-0.5">
                {!collapsed && <p className="px-2.5 pt-3 pb-1 text-[9px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50">{group}</p>}
                {items.map((item) => {
        ...
        {!collapsed && (
          <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-sidebar-border bg-sidebar">
            <p className="text-[9px] text-muted-foreground/60 text-center uppercase tracking-[0.15em]">NÈKO · Mike Nichols · v3.1</p>
          </div>
        )}
      </aside>
